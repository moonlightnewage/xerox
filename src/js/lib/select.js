export default function createSpinner(multipleSelectClass) {
    ...
    $(selects).each(function () {
        ...
        $(self).wrap('<div class="multiple-select"></div>').before('<div class="multiple-select__placeholder"><span class="multiple-select__placeholder-text">' + ($(options[0]).html()) + '</span></div>').after('<ul class="multiple-select__list"></ul>');
        ...
        $(switcher).append('<span class="icon__arrow-up"></span>').append('<span class="icon__arrow-down"></span>');
        $(switcher).children().wrap('<div class="icon"></div>')
